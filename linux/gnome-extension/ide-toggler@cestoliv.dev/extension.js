...
import {
    ORDER_MODES, groupForState, buildRows, compactRow,
    detectTransitions, nextBlinkSet,
} from './lib/aggregator.js';
import {setCairo, makeStatusIcon, loopingTransition} from './lib/statusIcons.js';
import {IdeTogglerIndicator} from './lib/indicator.js';

const BUS_NAME = 'dev.cestoliv.IdeToggler';
const OBJECT_PATH = '/dev/cestoliv/IdeToggler';

const IFACE_XML = `
<node>
  <interface name="dev.cestoliv.IdeToggler">
...
function windowTypeToString(type) {
    ...
    case Meta.WindowType.MODAL_DIALOG: return 'modal_dialog';
    ...
    case Meta.WindowType.DROPDOWN_MENU: return 'dropdown_menu';
    case Meta.WindowType.POPUP_MENU: return 'popup_menu';
    ...
    case Meta.WindowType.OVERRIDE_OTHER: return 'override_other';
...
export default class IdeTogglerExtension extends Extension {
    enable() {
        ...
        this._settingsPath = GLib.build_filenamev([
            GLib.get_user_config_dir(), 'ide-toggler.json']);
        this._settings = this._loadSettings();

        // --- window tracking ---
        const display = global.display;
        this._signalIds.push([display, display.connect('window-created', (_d, win) => {
        ...
        this._signalIds.push([Main.layoutManager, Main.layoutManager.connect(
            'monitors-changed', () => this._positionPanel())]);

        // --- backstop refresh (5s) ---
        this._backstopId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 5, () => {
            this._reloadSessions();
            this._refreshUi();
    ...
    _loadSettings() {
        try {
            ...
            if (ORDER_MODES.includes(obj.orderMode))
                s.orderMode = obj.orderMode;
            if (typeof obj.muted === 'boolean')
                s.muted = obj.muted;
            if (typeof obj.compactMode === 'boolean')
                s.compactMode = obj.compactMode;
            if (typeof obj.hidden === 'boolean')
                s.hidden = obj.hidden;
            if (Number.isFinite(obj.posX))
                s.posX = obj.posX;
            if (Number.isFinite(obj.posY))
                s.posY = obj.posY;
            return s;
        } catch (_e) {
            return this._defaultSettings();
    ...
    _setupSessionMonitor() {
        try {
            ...
            this._sessionMonitor = dir.monitor_directory(Gio.FileMonitorFlags.NONE, null);
            this._signalIds.push([this._sessionMonitor, this._sessionMonitor.connect(
                'changed', () => this._scheduleSessionReload())]);
        } catch (e) {
    ...
    _scheduleSessionReload() {
        if (this._sessionDebounceId)
            return;
        this._sessionDebounceId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 200, () => {
            this._sessionDebounceId = 0;
            this._reloadSessions();
            this._refreshUi();
    ...
    _onHandlePress(event) {
        ...
        try {
            if (global.stage.grab)
                this._dragGrab = global.stage.grab(this._handle);
        } catch (_e) {
            this._dragGrab = null;
        }
    ...
    _buildFooter() {
        this._footer = new St.BoxLayout({style_class: 'ide-toggler-footer', x_expand: true});
        ...
        this._handle.connect('button-press-event', (_a, ev) => this._onHandlePress(ev));
    ...
    _buildSettingsMenu() {
        this._settingsMenu = new PopupMenu.PopupMenu(this._gearBtn, 0.5, St.Side.BOTTOM);
        ...
        this._orderItems = {};
        const orderHeader = new PopupMenu.PopupMenuItem('Order', {reactive: false});
        ...
        for (const mode of ORDER_MODES) {
            ...
            item.setOrnament(this._settings.orderMode === mode
        ...
        this._muteItem = new PopupMenu.PopupSwitchMenuItem('Mute sound', this._settings.muted);
    ...
    _toggleSettingsMenu() {
        if (this._muteItem)
            this._muteItem.setToggleState(this._settings.muted);
        for (const m of ORDER_MODES) {
            this._orderItems[m]?.setOrnament(
    ...
    _positionPanel() {
        ...
        let x, y;
        if (Number.isFinite(this._settings.posX) && Number.isFinite(this._settings.posY)) {
    ...
    _refreshUi() {
        ...
        for (const r of rows)
            newStates.set(r.window.id, r.state);
        ...
        if (transitions.length && !this._settings.muted)
            this._playChime();
        ...
        if (this._settings.compactMode) {
            ...
            if (row)
                this._content.add_child(this._makeRow(row, showBadges));
            return;
        }
        ...
        const headerLabels = {needs: 'Needs you', working: 'Working', idle: 'Idle'};
        const grouped = {needs: [], working: [], idle: []};
        for (const r of rows)
            grouped[groupForState(r.state)].push(r);

        for (const g of order) {
            const groupRows = grouped[g];
            if (groupRows.length === 0)
                continue;
            this._content.add_child(this._makeGroupHeader(headerLabels[g], groupRows.length));
    ...
    _makeGroupHeader(label, count) {
        const box = new St.BoxLayout({style_class: 'ide-toggler-group-header', x_expand: true});
    ...
    _makeCompactHeader(rows) {
        const box = new St.BoxLayout({style_class: 'ide-toggler-compact-header', x_expand: true});
        const tally = {needs: 0, working: 0, idle: 0};
        for (const r of rows)
            tally[groupForState(r.state)] += 1;
        const order = ['needs', 'working', 'idle'];
    ...
    _makeRow(row, showBadge) {
        ...
        if (isQuiet)
            btn.add_style_class_name('ide-toggler-row-quiet');
        ...
        const hbox = new St.BoxLayout({style_class: 'ide-toggler-row-box', x_expand: true});
        ...
        if (icon._animate) {
            ...
            icon.connect('notify::mapped', () => {
                if (icon.mapped && icon.get_transition('spin') === null &&
        ...
        if (showBadge) {
            trailing.add_child(new St.Label({
        ...
        if (this._blinking.has(row.window.id)) {
            ...
            blinkBg.opacity = 77;
            const t = loopingTransition('opacity', 77, 15, 650,
